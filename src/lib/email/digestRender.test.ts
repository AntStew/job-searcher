...
describe("edge renderDigestHtml", () => {
  it("renders the job card with score, reasoning, and links", () => {
    const html = renderDigestHtml([JOB], UNSUB, SETTINGS, "rent dueeeee");
  ...
  it("falls back to a neutral heading without a taunt", () => {
    expect(renderDigestHtml([JOB], UNSUB, SETTINGS)).toContain("Your new matches");
  });

  it("escapes HTML in agent-supplied fields", () => {
    const hostile = { ...JOB, title: `<script>alert("pwn")</script>`, company: `a & b <i>` };
    const html = renderDigestHtml([hostile], UNSUB, SETTINGS);
  ...
  it("pluralizes the count line", () => {
    expect(renderDigestHtml([JOB], UNSUB, SETTINGS)).toContain("1 job cleared");
    expect(renderDigestHtml([JOB, { ...JOB, url: "https://example.com/2" }], UNSUB, SETTINGS)).toContain(
      "2 jobs cleared",
    );
  });
});
...
describe("React JobDigestEmail (Node sender)", () => {
  ...
  it("falls back to a neutral heading without a taunt", async () => {
    const html = await render(
      JobDigestEmail({ jobsList: [JOB], unsubscribeUrl: UNSUB, settingsUrl: SETTINGS }),
    );
