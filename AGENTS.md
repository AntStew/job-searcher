...
- `src/lib/pipeline/searchAndMatchForUser.ts` — ONE Sonnet call does search + scoring together (`web_search_20260209` server tool + custom `submit_job_matches` tool, `tool_choice: auto` so it can search first). Feeds known jobs into the prompt as an exclusion list, and liked/disliked history to steer taste. Deliberately capped: 4 web searches, ≤10 results, and a hard 100s timeout on the API call (`maxRetries: 0`) — Supabase free-plan edge workers are killed at ~150s wall clock (measured), so the search must finish inside that with room to save + email; uncapped runs died silently. Benign zero-result runs (timeout, cutoff, nothing found) set `cameUpEmpty`; scheduled runs then count as delivered and send a friendly "no new matches today" email (`sendEmptyRunEmail`) instead of the failure alert — retrying them would spam an empty-run note every half hour until the catch-up window closes
...
- The edge function's batch path answers the cron trigger in ~1s and dispatches one self-request per due user (each child gets its own worker and wall-clock budget, runs in `EdgeRuntime.waitUntil`) — never make the trigger wait on a search; that's what caused the GitHub "run failed" alerts. Overlapping fires are safe: a lock (`run_started_at`) younger than 10 min skips the user, older gets cleared by the batch watchdog with a "was interrupted" error so the slot retries
...
- Resend sends from the verified `unemploymentfinalboss.com` domain (`EMAIL_FROM`) — deliverable to any recipient
